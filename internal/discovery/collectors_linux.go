...
func (c *Collector) collectSystem(ctx context.Context, r *Result) {
	...
	if err == nil {
		for _, line := range strings.Split(string(b), "\n") {
			...
			case "NAME":
				r.System.OS.Name = v
			case "VERSION_ID":
				r.System.OS.VersionID = v
			case "VERSION":
				r.System.OS.Version = v
			case "VERSION_CODENAME":
				r.System.OS.Codename = v
			}
		}
	} else {
		addObservation(&r.Unknowns, "OS_UNKNOWN", "system", err.Error())
	}
	...
	if out, err := output(c, ctx, "df", "-B1", "/"); err == nil {
		r.System.RootFS = parseDF(string(out))
	} else {
		addObservation(&r.Unknowns, "ROOTFS_UNKNOWN", "system", err.Error())
	}
}
...
func parseDF(s string) Filesystem {
	...
	x := strings.Fields(lines[len(lines)-1])
	if len(x) >= 6 {
		f.Filesystem = x[0]
		f.SizeBytes, _ = strconv.ParseUint(x[1], 10, 64)
		f.UsedBytes, _ = strconv.ParseUint(x[2], 10, 64)
		f.AvailableBytes, _ = strconv.ParseUint(x[3], 10, 64)
		f.Mountpoint = x[5]
	}
	return f
}
...
func (c *Collector) collectNetwork(ctx context.Context, r *Result) {
	...
	if err := jsonOut(c, ctx, &links, "ip", "-j", "link"); err == nil {
		...
		addObservation(&r.Unknowns, "NETWORK_LINKS_UNKNOWN", "network", err.Error())
	}
		...
		for _, a := range addrs {
			for _, x := range a.AddrInfo {
				for i := range r.Network.Interfaces {
					if r.Network.Interfaces[i].Name == a.Ifname {
						r.Network.Interfaces[i].Addresses = append(r.Network.Interfaces[i].Addresses, Address{Address: x.Local, PrefixLength: x.Prefix, Family: x.Family})
					}
				}
			}
		}
	...
	if err := jsonOut(c, ctx, &routes, "ip", "-j", "route", "show", "default"); err == nil && len(routes) > 0 {
		r.Network.DefaultGateway = routes[0].Gateway
		r.Network.ExternalInterface = routes[0].Dev
		r.Network.IPv4 = true
	} else if err != nil {
		addObservation(&r.Unknowns, "DEFAULT_ROUTE_UNKNOWN", "network", err.Error())
	}
	...
	r.Network.IPv4 = has4 || r.Network.DefaultGateway != ""
	r.Network.IPv6 = has6
	...
	if b, err := os.ReadFile("/etc/resolv.conf"); err == nil {
		for _, l := range strings.Split(string(b), "\n") {
			...
			if len(f) >= 2 && f[0] == "nameserver" {
				r.Network.DNS.Resolvers = append(r.Network.DNS.Resolvers, f[1])
			}
		}
		r.Network.DNS.Source = "/etc/resolv.conf"
		r.Network.DNS.Active = len(r.Network.DNS.Resolvers) > 0
	} else {
		addObservation(&r.Unknowns, "DNS_UNKNOWN", "network", err.Error())
	}
}
...
func (c *Collector) collectSSH(ctx context.Context, r *Result) {
	...
	if socketState := text(c, ctx, "systemctl", "is-active", "ssh.socket"); socketState == "active" {
		r.SSH.Architecture = "socket-activated"
		addObservation(&r.Observations, "SSH_SOCKET_ACTIVATION", "ssh", "ssh.socket is active")
	} else {
	...
	for _, l := range strings.Split(s, "\n") {
		...
		case "port":
			if n, e := strconv.Atoi(f[1]); e == nil {
				r.EffectivePorts = appendUnique(r.EffectivePorts, n)
			}
		case "passwordauthentication":
			v := strings.EqualFold(f[1], "yes")
			r.PasswordAuthentication = &v
		case "pubkeyauthentication":
			v := strings.EqualFold(f[1], "yes")
			r.PubkeyAuthentication = &v
		case "permitrootlogin":
			r.PermitRootLogin = f[1]
		}
	}
}
...
func (c *Collector) collectServices(ctx context.Context, r *Result) {
	names := []string{"ssh.service", "ssh.socket", "docker.service", "mihomo.service", "mita.service", "fail2ban.service"}
	for _, n := range names {
		if s := text(c, ctx, "systemctl", "show", "-p", "LoadState", "-p", "ActiveState", "-p", "SubState", "-p", "UnitFileState", n); s != "" {
			sv := Service{Name: n, Exists: true}
			for _, l := range strings.Split(s, "\n") {
				p := strings.SplitN(l, "=", 2)
				if len(p) != 2 {
					continue
				...
				switch p[0] {
				case "LoadState":
					sv.Exists = p[1] == "loaded"
				case "ActiveState":
					sv.Active = p[1] == "active"
				case "SubState":
					sv.SubState = p[1]
				case "UnitFileState":
					sv.Enabled = p[1] == "enabled"
				}
			}
			if sv.Exists {
...
func listeners(c *Collector, ctx context.Context) []Listener {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		p := Listener{Address: addr, Port: port, Protocol: "tcp"}
		if len(f) >= 6 {
			p.Process = f[5]
			if strings.Contains(p.Process, "sshd") {
				p.Service = "sshd"
			}
		}
