...
func TestBuildPlanOwnedSSHIsCritical(t *testing.T) {
	...
	if a.Kind != ActionUpdateFile { t.Fatalf("expected typed update action: %#v", a) }
...
func TestBuildPlanUsesServiceForNonSocketSSH(t *testing.T) {
	...
	if p.Actions[0].Spec.SSH.Unit != "ssh.service" { t.Fatalf("expected ssh.service: %#v", p.Actions[0].Spec.SSH) }
}
